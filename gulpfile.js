...
gulp.task('rollup-bundle', (cb) => {
  ...
  .then(() => {
    ...
    const globals = {
      ...
      'wowjs': _.camelCase('wowjs'.replace('/','.'))
...
gulp.task('build:demo', () => {
  return execDemoCmd(`build --preserve-symlinks --prod --base-href /ngx-wow/ --deploy-url /ngx-wow/`, { cwd: `${config.demoDir}`});
});
...
gulp.task('build:demo-ssr', () => {
  return execDemoCmd(`build --preserve-symlinks --prod`, { cwd: `${config.demoDir}`})
    .then(() => execDemoCmd(`run ngx-wow-demo:server`, { cwd: `${config.demoDir}` }))
